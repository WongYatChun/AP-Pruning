...
lasso_valid_full <- function(ports, lambda0, lambda2, main_dir, sub_dir, adj_w, n_train_valid = 360, cvN = 3, kmin=5, kmax=50, parallel = 10) {
  ...
  # can change this loop to perform the full cross-validation
  for (i in cvN: cvN){
    ports_train = ports[-c(((i - 1) * n_valid + 1):(i * n_valid), (n_train_valid + 1):(nrow(ports))),]
    ports_valid = ports[((i - 1) * n_valid + 1):(i * n_valid),]
    lasso_cv_helper(ports_train, ports_valid, ports_test, lambda0, lambda2, main_dir, sub_dir, adj_w, nrow(ports), paste('cv', i, sep='_'), kmin,kmax,parallel)
  }
  
  # After pin-down the parameter, do another fit on the whole train+valid time period
  ports_train = ports[1:n_train_valid,]
  lasso_cv_helper(ports_train, NULL, ports_test, lambda0, lambda2, main_dir, sub_dir, adj_w, nrow(ports), 'full', kmin,kmax,parallel)
}

lasso_cv_helper <- function(ports_train, ports_valid, ports_test, lambda0, lambda2, main_dir, sub_dir, adj_w, n_total, cv_name, kmin=5, kmax = 50, parallel = 10) {
  ...
  source_path = paste(dirname(rstudioapi::getSourceEditorContext()$path), '/lasso.R', sep='')
